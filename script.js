document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- STATE VARIABLES ---
    let allUsers = [];     // The master list
    let activeDeck = [];   // The filtered list
    let currentSwipeIndex = 0;
    let swipeCount = parseInt(localStorage.getItem('pgX_swipeCount')) || 0;
    
    // --- INIT ---
    function init() {
        // Prevent modals from showing on load
        ...
        if (!localStorage.getItem('pgX_users_v2')) {
            generateMockUsers();
        }
        allUsers = JSON.parse(localStorage.getItem('pgX_users_v2'));
    ...
    function generateMockUsers() {
        ...
        localStorage.setItem('pgX_users_v2', JSON.stringify(mocks));
    }
    ...
    window.renderDeck = function(ignoreFilters = false) {
        ...
        if (ignoreFilters) {
            ...
            activeDeck = allUsers.filter(u => {
                ...
                if (noSmoke && u.smoker) return false;
                return true;
            });
        }

        // 3. Render Card or Empty State
        if (activeDeck.length > 0 && currentSwipeIndex < activeDeck.length) {
            ...
            // EMPTY STATE (Skeleton & Suggestions)
            zone.innerHTML = `
        ...
        if(grid) {
            grid.innerHTML = activeDeck.map(u => `
                <div class="grid-item" onclick="window.openUserModal('${u.name}', '${u.img}', '${u.id}')">
    ...
    window.saveProfile = function() {
        ...
        document.getElementById('save-modal').style.display='flex'; // Show success custom modal
    }
